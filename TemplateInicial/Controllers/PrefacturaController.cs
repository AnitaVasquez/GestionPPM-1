...
{
    [Autenticado]
    public class PrefacturaController : BaseAppController
    {
        // GET: Prefactura
        ...
        public async Task<PartialViewResult> IndexGrid(String search)
        {
            ...
            //Búsqueda
            var listado = CotizacionEntity.ListadoPrefacturasAprobadas();
        ...
        public ActionResult _VerAdjuntos(int? id)
        {
        ...
        {
            ViewBag.TituloModal = "Aprobar Prefactura";
            PrefacturaSAFIInfo modelo = CotizacionEntity.ConsultarPrefacturaSAFI(id.Value);
            return PartialView(modelo);
        }
        ...
        public ActionResult AprobacionFinalPrefactura(string listadoIDs, bool ajax = false)
        {
            try
            {
                ...
                #region Validacion facturas aprobadas
                string mensajeFacturasAprobadas = "Las siguientes Prefacturas: {0} ya se encuentran aprobadas.";

                string prefacturasAprobadas = string.Empty;
                //Verificar que no se encuentren aprobadas las facturas
                foreach (var id in ids)
                {
                    var prefactura = CotizacionEntity.ConsultarPrefacturaSAFI(id);
                    if (prefactura != null)
                    {
                        if (prefactura.aprobacion_final.Value)
                            prefacturasAprobadas += prefactura.numero_prefactura + ",";
                    }
                }

                mensajeFacturasAprobadas = string.Format(mensajeFacturasAprobadas, prefacturasAprobadas);

                if (!string.IsNullOrEmpty(prefacturasAprobadas))
                    return Json(new { Resultado = new RespuestaTransaccion { Estado = false, Respuesta = mensajeFacturasAprobadas } }, JsonRequestBehavior.AllowGet);
                #endregion

                #region Validacion facturas consolidadas
                string mensajeFacturasConsolidadas = "Las siguientes Prefacturas: {0} ya se encuentran consolidadas.";

                string prefacturasConsolidadas = string.Empty;
                //Verificar que no se encuentren aprobadas las facturas
                foreach (var id in ids)
                {
                    var prefactura = CotizacionEntity.ConsultarPrefacturaSAFI(id);
                    if (prefactura != null)
                    {
                        if (prefactura.prefactura_consolidada.Value)
                            prefacturasConsolidadas += prefactura.numero_prefactura + ",";
                    }
                }

                mensajeFacturasConsolidadas = string.Format(mensajeFacturasConsolidadas, prefacturasConsolidadas);

                if (!string.IsNullOrEmpty(prefacturasConsolidadas))
                    return Json(new { Resultado = new RespuestaTransaccion { Estado = false, Respuesta = mensajeFacturasConsolidadas } }, JsonRequestBehavior.AllowGet);
                #endregion

                var user = ViewData["usuario"] = System.Web.HttpContext.Current.Session["usuario"];
                var usuarioID = Convert.ToInt16(user);

                bool aprobado = CotizacionEntity.AprobacionFinalPrefactura(ids, usuarioID);
        ...
        public ActionResult ConsolidacionPrefacturas(string listadoIDs)
        {
            try
            {
                var ids = !string.IsNullOrEmpty(listadoIDs) ? listadoIDs.Split(',').Select(int.Parse).ToList() : new List<int> { int.Parse(listadoIDs) };
                ...
                RespuestaTransaccion resultado = CotizacionEntity.ConsolidarPrefactura(listadoIDs, usuarioID);
        ...
        {
            IGrid<PrefacturaSAFIInfo> grid = new Grid<PrefacturaSAFIInfo>(CotizacionEntity.ListadoPrefacturasAprobadas());
        ...
        public ActionResult DescargarReporteFormatoCSV()
        {
            ...
            var listado = (from item in CotizacionEntity.ListadoPrefacturasAprobadas()
                           select new object[]
                           {
        ...
        public ActionResult DescargarReporteFormatoPDF()
        {
            ...
            var results = CotizacionEntity.ListadoPrefacturasAprobadas();
