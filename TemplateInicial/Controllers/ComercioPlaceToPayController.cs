    ...
    public class ComercioPlaceToPayController : BaseAppController
    {
        ...
        public async Task<ActionResult> Create(ComercioPlaceToPay formulario, List<string> archivos, List<MidsComercioPlaceToPay> midsAsociados)
        {
            try
            {
                ...
                bool existeComercio = await ComercioPlaceToPayEntity.ExisteComercioPlaceToPayAsync(formulario.RUC);
                if (existeComercio)
                    return Json(new { Resultado = new RespuestaTransaccion { Estado = false, Respuesta = string.Format(Mensajes.MensajeValidacionComercioRepetido, formulario.RUC) } }, JsonRequestBehavior.AllowGet);
                ...
                if (!Validaciones.VerificaIdentificacion(formulario.IdentificacionPropietarioCuentaBancaria))
                    return Json(new { Resultado = new RespuestaTransaccion { Estado = false, Respuesta = string.Format(Mensajes.MensajeIdentificacionIncorrecto, formulario.IdentificacionPropietarioCuentaBancaria) } }, JsonRequestBehavior.AllowGet);
        ...
        public async Task<ActionResult> Edit(ComercioPlaceToPay formulario, List<string> archivos, List<MidsComercioPlaceToPay> midsAsociados)
        {
            ...
            {
                //Validar si el comercio ya existe
                bool existeComercio = await ComercioPlaceToPayEntity.ExisteComercioPlaceToPayAsync(formulario.RUC, formulario.IDComercioPlaceToPay);
                if (existeComercio)
                    return Json(new { Resultado = new RespuestaTransaccion { Estado = false, Respuesta = string.Format(Mensajes.MensajeValidacionComercioRepetido, formulario.RUC) } }, JsonRequestBehavior.AllowGet);
