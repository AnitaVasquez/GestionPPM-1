...
namespace GestionPPM.Entidades.Metodos
{
    public class CotizacionEntity 
    {
            ...
            using (var transaction = db.Database.BeginTransaction())
            {
                using (var context = new GestionPPMEntities())
                {
                    try
                    {
                        ...
                        if (cliente == null)
                            return new RespuestaTransaccion { Estado = false, Respuesta = Mensajes.MensajeTransaccionFallida + string.Format(Mensajes.MensajeClienteConsolidacionInexistente, nombreClienteBusqueda) };
                        ...
                        ListadoPrefactura.Add(new ListadoPresupuestoPrefacturaSAFI
                        {
                            ...
                            Cliente = cliente.nombre_comercial_cliente,
                            Identificacion = cliente.ruc_ci_cliente,
                            Correos = "-",
                            ...
                            CodigoProdcuto = "PG00002",
                            NombreProducto = "PG00002",
                            Bodega = "08",
                            IdBodega = 1,
                            Id_Producto = 1,
                            IdFormaPago = 7,
                            CodigoFormaPago = "20",
                            IdCentroCosto = 1,
                            CodigoCentroCosto = "43002",
                            Cantidad = resultadoConsolidacion.Cantidad,
                            PrecioUnitario = resultadoConsolidacion.PrecioUnitario,
                            Subtotal = resultadoConsolidacion.PrecioUnitario,
                            Descuento = resultadoConsolidacion.Descuento,
                            Total = resultadoConsolidacion.TotalPago,
                            Pago = 1,
        ...
        public static List<PrefacturaSAFIInfo> ListadoPrefacturasAprobadas()
        {
            List<PrefacturaSAFIInfo> listado = new List<PrefacturaSAFIInfo>();
            try
            {
                listado = db.ListadoPrefacturasAprobadas().ToList();
                return listado;
            }
            catch (Exception ex)
            {
