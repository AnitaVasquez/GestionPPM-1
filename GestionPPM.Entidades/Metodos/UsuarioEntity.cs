...
namespace GestionPPM.Entidades.Metodos
{
    public static class UsuarioEntity
    {
        ...
        public static IEnumerable<SelectListItem> ObtenerListadoUsuarios(string seleccionado = null, bool interno = true)
        {
            List<SelectListItem> listado = new List<SelectListItem>();
            try
            {
                string filtroTipoUsuairo = interno ? "INTERNO" : "EXTERNO";
                listado = db.ListadoUsuarios().Where(s=> s.Tipo_Usuario == filtroTipoUsuairo).OrderByDescending(c => c.Id).Select(c => new SelectListItem
                {
                    Text = c.Nombres_Completos,
                    Value = c.Id.ToString(),
                }).ToList();
